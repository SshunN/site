...
function getCart(){var c = localStorage.getItem('cart');if(c != null){ cart = JSON.parse(c); }}
  function changePage(id)
  {
    ...
    if(last != item){last.className = 'nav-link';item.className = 'navbar-brand';}
  }
  getCart();
  ...
  setHeader(array(new HeaderItem("goodH", "goodsPage.php", "Товары"), new HeaderItem("serviceH", "servicePage.php", "Услуги"),
  new HeaderItem("aboutH", "about.php", "О нас"), new HeaderItem("partsH", "partners.php", "Партнёры"),
  new HeaderItem("cartH", "", "", "openCart()")));
  inicializePage();
?>
